...
public abstract class Expression {
  ...
  public static Expression compare(String op, Expression a, Expression b) {
    ...
    switch (op) {
      case "=":
      case "==":
        return new EqualExpression(a, b);
      case "!=":
      case "<>":
        return new UnequalExpression(a, b);
      case ">=":
        return new GreaterThanEqualExpression(a, b);
      case ">":
        return new GreaterThanExpression(a, b);
      case "<=":
        return new LessThanEqualExpression(a, b);
      case "<":
  ...
  public static Expression function(String name, Expression... children) {
    ...
    switch (name) {
      // Regular functions
      case "UPPER":
        return new NamedFunction.UpperFunction(children);
      case "LOWER":
        return new NamedFunction.LowerFunction(children);
      case "REPLACE":
        return new NamedFunction.ReplaceFunction(children);
      case "ROUND":
        return new NamedFunction.RoundFunction(children);
      case "CEIL":
        return new NamedFunction.CeilFunction(children);
      case "FLOOR":
        return new NamedFunction.FloorFunction(children);
      case "NEGATE":
        return new NamedFunction.NegateFunction(children);
      // Aggregates
      case "FIRST":
        return new AggregateFunction.FirstAggregateFunction(children);
      case "SUM":
        return new AggregateFunction.SumAggregateFunction(children);
      case "COUNT":
        return new AggregateFunction.CountAggregateFunction(children);
      case "MAX":
        return new AggregateFunction.MaxAggregateFunction(children);
      case "MIN":
        return new AggregateFunction.MinAggregateFunction(children);
      case "AVG":
        return new AggregateFunction.AverageAggregateFunction(children);
      case "VARIANCE":
        return new AggregateFunction.VarianceAggregateFunction(children);
      case "STDDEV":
        return new AggregateFunction.StdDevAggregateFunction(children);
      case "RANGE":
        return new AggregateFunction.RangeAggregateFunction(children);
      case "RANDOM":
        return new AggregateFunction.RandomAggregateFunction(children);
      case "LAST":
        return new AggregateFunction.LastAggregateFunction(children);
    }
  ...
  public static boolean toBool(DataBox d) {
    switch (d.getTypeId()) {
      ...
      case INT:
        return d.getInt() != 0;
      case STRING:
        return !d.getString().equals("");
      case FLOAT:
        return d.getFloat() != 0.0;
      case BYTE_ARRAY:
        throw new UnsupportedOperationException("Cannot interpret byte array as true/false");
      default:
  ...
  public static Type resultType(List<Expression> children) {
    ...
    for (Expression child : children) {
      ...
      switch (type) {
        case STRING:
          throw new RuntimeException("Cannot convert string to numeric type.");
        case BYTE_ARRAY:
          throw new RuntimeException("Cannot convert byte array to numeric type");
        case BOOL:
          type = TypeId.INT; // Treat booleans as integer 0/1
        default:
          break;
      }
          ...
          for (int i = 1; i < this.children.size(); i++) {
            ...
            switch (ops.get(i - 1)) {
              ...
              case '%':
                ...
                break;
              default:
                throw new RuntimeException("Unexpected operator: " + ops.get(i - 1))
                    ;
            }
          }
  ...
  static class NegateExpression extends Expression {
    ...
    public DataBox evaluate(Record record) {
      ...
      switch (this.type.getTypeId()) {
        case INT:
          return new IntDataBox(-toInt(c));
        case LONG:
          return new LongDataBox(-toLong(c));
        case FLOAT:
          return new FloatDataBox(-toFloat(c));
        default:
          throw new RuntimeException("Unreachable code");
      }
    }
